...
class ApiClientBooks:NSObject{
    ...
    func RejectBorrowRequest(userId : String, bookId: UUID, completion: @escaping (Bool)->Void){
        let token = self.keychain.get(KeychainSwift.Keys.Token)
        let headers : HTTPHeaders = [
            "Authorization" : "Bearer \(token!)"
        ]
        ...
        Alamofire.request(Urls.RejectBorrowRequest,method:.post,parameters:params,encoding:URLEncoding.httpBody, headers: headers)
            .validate()
            .response { (response) in
                switch response.response?.statusCode{
                case 200?:
